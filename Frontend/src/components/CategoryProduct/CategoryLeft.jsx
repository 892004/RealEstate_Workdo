...
const CategoryLeft = () => {
  ...
  return (
    <section className="category-left h-full w-full relative py-10 flex flex-col items-center bg-[#b3876c] ">
      {/* Slider Window */}
      <div className="overflow-hidden h-full w-1/2 -translate-y-30 box-border -translate-x-70">
        <div
          ref={trackRef}
        ...
        >
          {slides.map((elem, idx) => (
            <div key={idx} className="min-w-[50%] min-h-full relative rounded-xl">
              <img
                src={elem.img}
              ...
              <h3 className="text-white absolute text-2xl mt-3 font-semibold flex items-center">
                {elem.title}  <span className=" px-25 font-bold text-2xl cursor-pointer"><HiArrowLongRight />
</span>
              </h3>
            </div>
          ))}
      ...
      {/* Buttons */}
      <div className="flex items-center justify-center absolute top-70 left-30 gap-120 ">
        <div className="line h-0.5 w-118 opacity-50 bg-white absolute top-50 -translate-y-47"> </div>
        <button
          onClick={prev}
          className="px-3 relative flex items-center justify-center text-white rounded-full border border-white text-xl cursor-pointer  "
        >
          ←
        </button>
        ...
        <button
          onClick={next}
          className="px-3 text-white  rounded-full border border-white text-xl cursor-pointer "
        >
          →
        </button>
      </div>
